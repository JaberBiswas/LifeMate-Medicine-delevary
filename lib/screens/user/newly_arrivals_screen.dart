...
class NewlyArrivalsScreen extends StatelessWidget {
  const NewlyArrivalsScreen({super.key});
      ...
      body: SafeArea(
        child: Consumer<ProductProvider>(
          builder: (context, provider, child) {
            ...
            List<MedicineModel> newlyArrived = List<MedicineModel>.from(medicines)
              ..sort((a, b) {
                try {
            ...
            return Column(
              children: [
                ...
                _buildHeader(newlyArrived.length),
                Expanded(
                  child: newlyArrived.isEmpty
                      ? _buildEmptyState()
                      : ListView.builder(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          itemCount: newlyArrived.length,
                          itemBuilder: (context, index) {
                            final medicine = newlyArrived[index];
                            return MedicineCard(
                              medicine: medicine,
                              onTap: () {
                                Navigator.push(
